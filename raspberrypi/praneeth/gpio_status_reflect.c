...
static ssize_t led_read(struct file *filep,char __user *buffer,size_t len, loff_t *offset) {
	int status;
	status=gpio_get_value(GPIO_STATUS);
...
static int __init led_init(void) {
    ...
    //request for gpio 
    if(!gpio_is_valid(GPIO_STATUS)) {
    	pr_err("LED: Invalid GPIO %d\n", GPIO_STATUS);
	return -ENODEV;
    }

    if(gpio_request(GPIO_STATUS, DRIVER_NAME)) {
    	pr_err("INPUT : request failed %d\n", GPIO_STATUS);
	return -EBUSY;
    }

    gpio_direction_output(GPIO_LED, 0);  // Set as output, initial LOW
    gpio_direction_input(GPIO_STATUS);

    // Character device registration
    major = register_chrdev(0, DEVICE_NAME, &fops);
    if (major < 0) {
        gpio_free(GPIO_LED);
	gpio_free(GPIO_STATUS);
