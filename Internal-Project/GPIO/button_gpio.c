...
static int gpio_probe(struct platform_device *gpio_button)
{
	...
	if(devm_request_irq(dev,irq_num,isr_handler,IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING,"isr_handler",NULL))
	{
...
static int gpio_remove(struct platform_device *gpio_buttom)
{
