...
#define GPIO_LED1 (18 +512)
#define GPIO_LED2 (19 + 512)
...
static int __init led_init(void) {
    ...
    if (gpio_request(GPIO_LED1, DRIVER_NAME)) {
        pr_err("LED: Failed to request GPIO %d\n", GPIO_LED1);
        return -EBUSY;
    }
    if (gpio_request(GPIO_LED2, DRIVER_NAME)) {
        pr_err("LED: Failed to request GPIO %d\n", GPIO_LED2);
        return -EBUSY;
    }
    ...
    if (major < 0) {
        gpio_free(GPIO_LED);
        gpio_free(GPIO_LED1); gpio_free(GPIO_LED2);
    ...
    if (IS_ERR(led_class)) {
        ...
        gpio_free(GPIO_LED);gpio_free(GPIO_LED1);gpio_free(GPIO_LED2);
