...
static ssize_t led_write(struct file *filep, const char *buffer, size_t len, loff_t *offset) {
    ...
    if (msg[0] == '1') {
        gpio_set_value(GPIO_STATUS_LED, 1);
        pr_info("LED: ON\n");
} else if (msg[0] == '0') {
        gpio_set_value(GPIO_STATUS_LED, 0);
        pr_info("LED: OFF\n");
    }
        int val=gpio_get_value(GPIO_STATUS_LED);
        if(val)
        {
...
static int __init led_init(void) {
 ...
 if (!gpio_is_valid(GPIO_STATUS_LED)) {
        pr_err("LED: Invalid GPIO %d\n", GPIO_STATUS_LED);
        return -ENODEV;
    }
    ...
    if (gpio_request(GPIO_STATUS_LED, DRIVER_NAME)) {
        pr_err("LED: Failed to request GPIO %d\n", GPIO_STATUS_LED);
        return -EBUSY;
    }
    ...
    if (IS_ERR(led_class)) {
            ...
            gpio_free(GPIO_LED);gpio_free(GPIO_STATUS_LED);
