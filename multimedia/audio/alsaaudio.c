...
#define PCM_CAPTURE_DEVICE "plughw:0,0"   // USB headset capture
#define PCM_PLAYBACK_DEVICE "plughw:0,0"  // USB headset playback
...
int main() {
    ...
    // Open capture device
    if ((err = snd_pcm_open(&capture_handle, PCM_CAPTURE_DEVICE,
    ...
    if (!fp) {
        fprintf(stderr, "Cannot open recorded file\n");
        return 1;
    }

    while ((err = fread(buffer, sizeof(int16_t), frames * channels, fp)) > 0) {
