...
/*--------------------------------------------------------
  Set rotation (NEW FUNCTION)
  rot = 0: default portrait
  rot = 1: left→right (90° CW)
  rot = 2: upside-down
  rot = 3: right→left (270° CW)
--------------------------------------------------------*/
static void ili9225_set_rotation(struct ili9225 *lcd, int rot)
{
    switch(rot) {
        case 0:
            ...
            break;
        case 1: // left→right
            ili9225_write_reg(lcd, 0x03, 0x1018);
            break;
        case 2:
            ili9225_write_reg(lcd, 0x03, 0x1000);
            break;
        case 3: // right→left
            ili9225_write_reg(lcd, 0x03, 0x1028);
            break;
    }
}
                         ...
                         loff_t *off)
{
    ...
    // Example: rotate left→right before drawing
    ili9225_set_rotation(g_lcd, 1);   // <--- CHANGE ROTATION HERE (0-3)
...
MODULE_DESCRIPTION("ILI9225 Image Display Driver with Rotation Support");
