...
/* Check UART loopback: write a byte and read back */
static void check_uart_loopback(void)
{
    char test_byte = 'A';
    char read_byte = 0;
    ...
    // Write test byte
    iowrite32(test_byte, uart_base + UART_DR);
    msleep(10);  // wait a bit

    // Read from DR (simplified)
    read_byte = ioread32(uart_base + UART_DR) & 0xFF;
...
static void mouse_event(struct input_handle *handle,
                        ...
                        int value)
{
    if(!mouse_connected || !uart_ready)
        return;  // Ignore if mouse not connected or UART not looped
                         ...
                         const struct input_device_id *id)
{
    ...
    pr_info("Mouse connected (UART ready: %d)\n", uart_ready);
    return 0;
}
...
MODULE_DESCRIPTION("Mouse Quadrant LED Driver using ioremap GPIO17 + /dev + UART loopback check");
