...
static ssize_t led_read(struct file *filep,char *buffer,size_t len,loff_t *offset){
	int status;
	status=gpio_get_value(check);
	if(copy_to_user(buffer,&status,sizeof(int)))
			return -EFAULT;
	pr_info("status=%d\n",check);
return len;
}
...
static int __init led_init(void) {
    ...
    gpio_request(check,DRIVER_NAME);

    gpio_direction_output(GPIO_LED, 0);  // Set as output, initial LOW
    gpio_direction_output(check, 0);  // Set as output, initial LOW
