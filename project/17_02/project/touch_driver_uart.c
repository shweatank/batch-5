                          ...
                          const char *buf, size_t count)
{
    ...
    strncpy(stored_name, buf, count);
    stored_name[count] = '\0';
    pr_info("Sysfs: stored name = %s\n", stored_name);
    return count;
}

static struct kobj_attribute name_attribute =
    __ATTR(name, 0220, NULL, name_store);
...
static int __init touch_driver_init(void)
{
    ...
    pr_info("Mouse Touch LED + Sysfs Driver Loading...\n");
    ...
    mouse_kobj = kobject_create_and_add("mouse_touch", kernel_kobj);
    if(!mouse_kobj){
    ...
    ret = sysfs_create_file(mouse_kobj, &name_attribute.attr);
    if(ret){
    ...
    pr_info("Sysfs attribute created at /sys/kernel/mouse_touch/name\n");
...
static void __exit touch_driver_exit(void)
{
    ...
    destroy_workqueue(led_wq);
    led_off();
    if(mouse_kobj) kobject_put(mouse_kobj);
    if(gpio_base) iounmap(gpio_base);
    pr_info("Mouse Touch LED + Sysfs Driver Unloaded\n");
}
...
MODULE_DESCRIPTION("Mouse Quadrant LED Driver using ioremap GPIO17 + sysfs name");
