...
{
    u8 buf[2]; 
    buf[0] = value >> 8;                  //Extracts higher 8 bits in buff[0]
    buf[1] = value & 0xFF;                //Extracts lower 8 bits in buff[1]
    return spi_write(lcd->spi, buf, 2);   //returns buff to write 
}
...
{
    ili9225_reset(lcd); //Resting the lcd to clear previous internal logics.

    ili9225_write_reg(lcd, 0x0001, 0x011C); //scan directing is form top to bottom
    ili9225_write_reg(lcd, 0x0002, 0x0100); //flipping for every line(row) of display

    // LANDSCAPE ENTRY MODE
    ili9225_write_reg(lcd, 0x0003, 0x1038); //incrementing the address counter horizontallly first

    ili9225_write_reg(lcd, 0x0008, 0x0808); //idle time" between frames that allows the electronics to reset.
    ili9225_write_reg(lcd, 0x000C, 0x0000); //Selecting internal clock
    ili9225_write_reg(lcd, 0x000F, 0x0B01); //Setting frequency ~ 60Hz

    ili9225_write_reg(lcd, 0x0010, 0x0A00); //power control1 (Setting Voltage)
    ili9225_write_reg(lcd, 0x0011, 0x1038); //starting to supply power
    msleep(50);                             // delay to get charge

    ili9225_write_reg(lcd, 0x0012, 0x1121); // Establishing baseline voltage for all pixels
//Flipping the voltage polarity and ensures positive and negative are equal
    ili9225_write_reg(lcd, 0x0013, 0x0063);
    ili9225_write_reg(lcd, 0x0014, 0x5A00);
    msleep(50);                             // stabilization gap

    ili9225_write_reg(lcd, 0x0007, 0x1017);// Logic ON, Display Output ON, Data Flow ON, and Gate Drivers ON. Go
    msleep(20);
}
...
/* setting the origins */
    ...
    ili9225_write_reg(lcd, 0x0036, LCD_HEIGHT - 1); //setting right edge
    ili9225_write_reg(lcd, 0x0037, 0);              //setting left edge
    ili9225_write_reg(lcd, 0x0038, LCD_WIDTH - 1);  //setting Bottom edge
    ili9225_write_reg(lcd, 0x0039, 0);              //setting top edge

    ili9225_write_reg(lcd, 0x0020, 0);             //Setting cursor to start of horizantal axis
    ili9225_write_reg(lcd, 0x0021, 0);             //Setting cursor to start of vertical axis

    gpiod_set_value(lcd->rs, 0);                  //setting low before sending address
    ili9225_write16(lcd, 0x0022);                //Sending address
    gpiod_set_value(lcd->rs, 1);                  //setting high after sending address

/* Filling backgroud with white colour*/
...
static void drawPixel(int x, int y, uint16_t color)
{
    ...
    gpiod_set_value(lcd->rs, 0);
    ili9225_write16(lcd, 0x0020);    //Sets Horiantal GRAM Address
    gpiod_set_value(lcd->rs, 1);
    ili9225_write16(lcd, y_rotated); // Use y_rotated for Gram Address Set

    gpiod_set_value(lcd->rs, 0);
    ili9225_write16(lcd, 0x0021);    //Sets Vertical GRAM Address
    gpiod_set_value(lcd->rs, 1);
    ili9225_write16(lcd, x_rotated); // Use x_rotated for Gram Address Set

    gpiod_set_value(lcd->rs, 0);
    ili9225_write16(lcd, 0x0022);    //Actual pixel colour is pushed
    gpiod_set_value(lcd->rs, 1);
    ili9225_write16(lcd, color);
}
...
{
    if (c < 32 || c > 127) return;   //ASCII lessthan 32 and greater than 127

    const uint8_t *bitmap = font8x8[c - 32];  //To get the character

    for (int row = 0; row < 8; row++)
    {
        for (int col = 0; col < 8; col++)
        {
            // Check the bit for the current pixel
            if (bitmap[row] & (1 << (7 - col)))
            {
                // Draw only a single pixel instead of a 2x2 block
...
static void drawString(int x, int y, const char *str, uint16_t color)
{
    for (int i = 0; str[i]; i++)
    {
        ...
        x += 9;                           //To move cursor to next charecter
    }
}
...
pr_info("Temperature=%dC\n",sensor_data->temperature);
pr_info("Humidity=%d%%\n",sensor_data->humidity);
  ili9225_fill(g_lcd, 0xFFFF);
snprintf(temp_str,sizeof(temp_str),"Temperature:%dC",sensor_data->temperature);
snprintf(hum_str,sizeof(hum_str),"Humidity:%d%%",sensor_data->humidity);
drawString(50,50,temp_str,0x0000);
drawString(50,59,hum_str,0x0000);
