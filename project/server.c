...
static u8 fp_search_second_person[] =  {0xEF,0x01,0xFF,0xFF,0xFF,0xFF,0x01,0x00,0x08,0x04,0x02,0x00,0x00,0xA3,0x00,0xB4};
static u8 fp_search[] = {0xEF,0x01,0xFF,0xFF,0xFF,0xFF,0x01,0x00,0x08,0x04,0x01,0x00,0x00,0xA3,0x00,0xB1};
...
{ 
	...
	while (!kthread_should_stop())
	{
		...
		if (ret > 0) {
			...
			else if((strcmp(rx,"I am new client")==0)&&(!first_scan))
			{
				

				uart_write_data(fp_search_second_person,sizeof(fp_search_second_person));//send search command
