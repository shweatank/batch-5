...
#define LED_PHYS_ADDR  0x3F200000  // Replace with your LED MMIO register
#define UART_PHYS_ADDR 0x3F201000  // Replace with your UART MMIO base
...
/* ---------------- MMIO LED ---------------- */
static void led_on(void)   { if(led_base) iowrite32(1, led_base); pr_info("LED ON\n"); }
static void led_off(void)  { if(led_base) iowrite32(0, led_base); pr_info("LED OFF\n"); }
...
static void uart_send_char(char c)
{
    ...
    while(!(ioread32(uart_base + 0x18) & (1 << 5)))  // TX ready bit (depends on UART)
        cpu_relax();
    iowrite32(c, uart_base);
}
...
static int ili9225_probe(struct spi_device *spi)
{
    ...
    /* MMIO LED / UART */
    led_base = ioremap(LED_PHYS_ADDR, 0x4);
    uart_base = ioremap(UART_PHYS_ADDR, 0x100);  // map 256 bytes for UART
...
MODULE_DESCRIPTION("SPI ILI9225 driver integrated with mouse events, MMIO LED, MMIO UART, and workqueue LCD display");
