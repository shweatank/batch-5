...
static struct gpio_desc *led17; // LED on GPIO17 for speed
                         ...
                         size_t count, loff_t *pos)
{
    ...
    */
    /* ---- LED17 Blink based on SPEED (Clear 3-Level Logic) ---- */
...
if (speed_cm_per_sec == 0) {
    ...
    blink_delay = 500;        // Slow blink (Low speed)
}
else if (speed_cm_per_sec <= 20) {
    blink_delay = 200;        // Moderate blink (Medium speed)
}
else {
    blink_delay = 50;         // Very fast blink (High speed)
}
...
MODULE_DESCRIPTION("HC-SR04 Driver with Speed-Based LED Blink on GPIO17 for Raspberry Pi 4");
