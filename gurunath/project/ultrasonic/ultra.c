...
static int measure_fn(void *data)
{
    while (!kthread_should_stop()) {
        ...
        /* Wait for ECHO to go high with timeout and stop check */
        ...
        /* Wait for ECHO to go low with timeout and stop check */
        timeout = jiffies + msecs_to_jiffies(100);
        while (gpio_read(ECHO)) {
            if (kthread_should_stop() || time_after(jiffies, timeout))
                goto skip_measure;
...
static int __init ultrasonic_init(void)
{
    ...
    printk(KERN_INFO "sudo mknod /dev/hcsr04_1 c %d 0\n", MAJOR(devno));
    printk(KERN_INFO "sudo chmod 666 /dev/hcsr04_1\n");
