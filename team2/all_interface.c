...
{
    ili9225_display_reset(lcd);

    ili9225_display_write_reg(lcd, 0x0001, 0x011C);
    ili9225_display_write_reg(lcd, 0x0002, 0x0100);

    // LANDSCAPE ENTRY MODE
    ili9225_display_write_reg(lcd, 0x0003, 0x1038);

    ili9225_display_write_reg(lcd, 0x0008, 0x0808);
    ili9225_display_write_reg(lcd, 0x000C, 0x0000);
    ili9225_display_write_reg(lcd, 0x000F, 0x0B01);

    ili9225_display_write_reg(lcd, 0x0010, 0x0A00);
    ili9225_display_write_reg(lcd, 0x0011, 0x1038);
    msleep(50);

    ili9225_display_write_reg(lcd, 0x0012, 0x1121);
    ili9225_display_write_reg(lcd, 0x0013, 0x0063);
    ili9225_display_write_reg(lcd, 0x0014, 0x5A00);
    msleep(50);

    ili9225_display_write_reg(lcd, 0x0007, 0x1017);
    msleep(20);
}
...
static void ili9225_display_fill(struct ili9225_display *lcd, u16 color)
{
    ...
    ili9225_display_write_reg(lcd, 0x0036, LCD_HEIGHT - 1);
    ili9225_display_write_reg(lcd, 0x0037, 0);
    ili9225_display_write_reg(lcd, 0x0038, LCD_WIDTH - 1);
    ili9225_display_write_reg(lcd, 0x0039, 0);

    ili9225_display_write_reg(lcd, 0x0020, 0);
    ili9225_display_write_reg(lcd, 0x0021, 0);
...
{
    struct ili9225_display *lcd = g_lcd;
...
static void sensor_work(struct work_struct *work)
{
 int i, j;
    ...
    char temp_str[25],hum_str[25]; //strings to store temperature and humidity
...
pr_info("Humidity=%d.%d%%\n",buf[0],buf[1]);// humidity -> buf[0]=integer part buf[1]=fraction part 
  ili9225_fill(g_lcd, 0xFFFF);// clear screen with white screen
...
/* ---------------------------------------------------- */
static int ili9225_dislay_probe(struct spi_device *spi)
{
