...
static void ili9225_reset(struct ili9225 *lcd)
{
    ...
    msleep(20);                        //wait for 20ums
    gpiod_set_value(lcd->reset, 1);    //again pull high(release reset)
...
{
    ili9225_reset(lcd);                         //Resetting the LCD to clear previous logics(if any)
    ...
    ili9225_write_reg(lcd, 0x0003, 0x1038);     //Tells address counter to increment horizantally(Land scape)

    ili9225_write_reg(lcd, 0x0008, 0x0808);     //Controls idle between frames (Allows electronics to reset)
    ili9225_write_reg(lcd, 0x000C, 0x0000);     //Selects internal clock 
    ili9225_write_reg(lcd, 0x000F, 0x0B01);     //Controls clock frequency (~60Hz)
    ...
    ili9225_write_reg(lcd, 0x0007, 0x1017);     //Enbales Internal logic , Display output
...
static void ili9225_fill(struct ili9225 *lcd, u16 color)
{
    ...
    ili9225_write_reg(lcd, 0x0036, LCD_HEIGHT - 1);  //Sets bottom boundary of display
    ili9225_write_reg(lcd, 0x0037, 0);               //Vertical start Address
    ili9225_write_reg(lcd, 0x0038, LCD_WIDTH - 1);   //Sets rightmost coloumn
    ili9225_write_reg(lcd, 0x0039, 0);               //Start column = 0(left side)
     //GRAM Address
    ili9225_write_reg(lcd, 0x0020, 0);               //Set column pointer to 0
    ili9225_write_reg(lcd, 0x0021, 0);               //Set row pointer to 0
//Start writing to GRAM (Pixel Memory)
    gpiod_set_value(lcd->rs, 0);                     //Setting low(command mode)
    ili9225_write16(lcd, 0x0022);                    //This is GRAM data register
    gpiod_set_value(lcd->rs, 1);                     //Switch to data mode

///////Fill Entire screen with (white) colour ///////
...
{
    struct ili9225 *lcd = g_lcd;     //global pointer to LCD device
    ...
    gpiod_set_value(lcd->rs, 0);        
    ili9225_write16(lcd, 0x0020);      //setting row position
    gpiod_set_value(lcd->rs, 1);
    ili9225_write16(lcd, y_rotated); // Use y_rotated for Gram Address Set

    gpiod_set_value(lcd->rs, 0);
    ili9225_write16(lcd, 0x0021);     //setting coloumn position
    gpiod_set_value(lcd->rs, 1);
...
{
int scale=2;
    if (c < 32 || c > 126)         //Allow printable characters
        return;
