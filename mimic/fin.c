...
static int mimic_fn(void *data)
{
	...
	while (!kthread_should_stop())	//runs until the module is unloaded
	{
		...
		if (uart_read_byte(&byte))
		{
			...
			//if (index >= 14)  // minimum GC07 packet
			if (index >= 9)   // we have length field now
			{
				int pkt_len = (buffer[7] << 8) | buffer[8];
				int total_len = pkt_len + 9;
				...
				switch (buffer[9])
				{
					case 0x01:  // GET_IMAGE
						...
						print_hex_dump(KERN_INFO, "PKT: ",
               DUMP_PREFIX_OFFSET, 16, 1,
               buffer, total_len, true);
					...
					case 0x06:  // FP_ADD
						...
						pr_info("MIMIC: Stored image size = %d\n", image_size);
						uart_send(success_ack, sizeof(success_ack));
						break;
					...
					case 0x04:  // FP_SEARCH
						...
						if (search_len == image_size &&
								memcmp(registered_image, buffer + 10, image_size) == 0)
						{
							pr_info("MIMIC: MATCHED\n");
							uart_send(success_ack, sizeof(success_ack));
						}
