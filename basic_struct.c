...
//	snprintf(user_buffer,sizeof(user_buffer),"%d",res);
//	printk(KERN_INFO "Result=%s\n",user_buffer);
...
int num1,num2;
char ch;
num1=*(int*)kernel_buffer;
num2=*(int*)(kernel_buffer+4);
ch=*(char*)(kernel_buffer+8);
printk(KERN_INFO "num1=%d num2=%d ch=%c\n",num1,num2,ch);
switch(ch){
...
int temp,rev=0;
for(temp=res;temp;temp=temp/10){
int r=temp%10;
rev=rev*10+r;
}
int i=0;
for(temp=rev;temp;temp=temp/10,i++)
kernel_buffer[i]=temp%10;
kernel_buffer[i]='\0';
*(int*)kernel_buffer=res;
