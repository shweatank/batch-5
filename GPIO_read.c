...
static ssize_t led_read(struct file *fp, const char *buffer,size_t len, loff_t *offset){
	
	if(len<sizeof(int))
		return -EINVAL;
	int n=get_gpio_value(GPIO_IP);
	if(copy_to_user(buffer,&n,sizeof(int)))
		return -EFAULT;
