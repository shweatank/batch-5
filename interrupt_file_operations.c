...
struct req cal={100,200,0};

static void operations(void);
void static operations(void)
{
	printk("result=%d",cal.c);
}
...
static irqreturn_t keyboard_isr(int irq, void *dev_id)
{
   ...
   // printk(KERN_INFO "%s\n",key);
     ...
     switch(*key)
           {
                   case 'A':cal.c=cal.a+cal.b;
                               break;
                   case 'S':cal.c=cal.a-cal.b;
                             break;
                   case 'M':cal.c=cal.a*cal.b;
                             break;
		   default:
			    return IRQ_HANDLED;
                          ...
                          loff_t *offset)
{
    ...
    if (copy_to_user(user_buffer,&cal,sizeof(cal)))
        return -EFAULT;


      printk(KERN_INFO "gurunath\n");
...
//     printk(KERN_INFO"read from user=%d %d\n",cal.a,cal.b);
 //    printf("%d\n",cal.c);
