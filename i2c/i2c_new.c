...
static int __init i2c_init(void){
	...
	i2c_base=ioremap(Base,size);
        ...
        writel((readl(i2c_base+DIV)&(0xffff0000))|1500,i2c_base+DIV);
	writel((0<<4)|(0<<5),i2c_base+con);
	writel((1<<7)|(1<<15),i2c_base+con);
	/*writel(2,i2c_base+DLEN);
	 *val=read(i2c_base+con);
	 *
	 */
	writel((0x3c),i2c_base+SADD);
...
///	pr_info("wrote %c into fifo for slave\n",ch);
	//ch='b';
	//i2c_write(ch);
	val=readl(i2c_base+stat);
	if(val&(1<<8))
		pr_info("ack error\n");
	else
		pr_info("oled responding\n");
	//pr_info("wrote %c to slave\n",ch);
//	int x=i2c_read();
//	pr_info("read %c from slave\n",x);
return 0;
}
static void i2c_write(char mode,char ch){
	pr_info("in write func of i2c\n");
	int val=readl(i2c_base+con),t=10000;
	val|=(1<<7);
	val&=~(0<<0);
	...
	while(!(readl(i2c_base+stat)&(1<<4)) && t>0){
		t--;
		cpu_relax();
	}
	t=10000;
	writel(mode,i2c_base+FIFO);
	while(!(readl(i2c_base+stat)&(1<<4)) && t>0){
		t--;
		cpu_relax();
	}
