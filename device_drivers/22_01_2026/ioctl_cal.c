...
static long calc_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    if (copy_from_user(req, (void __user *)arg, sizeof(struct calc_req)))
        return -EFAULT;
    ...
    if (copy_to_user((void __user *)arg, req, sizeof(struct calc_req)))
        return -EFAULT;
...
static int __init calc_init(void)
{
	...
	req=kmalloc(sizeof(struct calc_req),GFP_ATOMIC);
