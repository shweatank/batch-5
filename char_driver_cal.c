                           ...
                           loff_t *offset)
{
    ...
    if (copy_from_user(kernel_buffer,
                       ...
                       bytes_to_copy))
        return -EFAULT;
	printk(KERN_INFO "kernel buff:%s\n",kernel_buffer);
	int a,b,res,i=0,flag=0;
	char ch;
		for(int i=0;i<strlen(kernel_buffer);i++)
		{
			if(kernel_buffer[i]==','){flag=1;continue;}
			else if((kernel_buffer[i]=='+')||(kernel_buffer[i]=='-')||(kernel_buffer[i]=='*')||(kernel_buffer[i]=='/'))
			{
				ch=kernel_buffer[i];
			}
			else if((kernel_buffer[i]>='0')&&(kernel_buffer[i]<='9')&&(flag==0))
			{
				a=(a*10)+(kernel_buffer[i]-48);
			}
			else
			{
				b=(b*10)+(kernel_buffer[i]-48);
			}
		}
	...
	int t=0,k=0;
	if(res<0){kernel_buffer[k++]='-';res=-res;}
	t=res;
        while(t)
        {
                int d=t%10;
                kernel_buffer[k++]=d+48;
                t/=10;
	}
	if(res==0){kernel_buffer[k++]=48;}
	kernel_buffer[k]='\0';
	printk(KERN_INFO,"res:%s\n",kernel_buffer);	
	char *l=kernel_buffer,*r=kernel_buffer+(strlen(kernel_buffer)-1);
        if(*l=='-'){l++;}
	while(l<r)
        {
               char t=*l;
