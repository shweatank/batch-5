...
#define DEVICE_NAME "string_driver"
#define BUF_SIZE    128
...
void cal_result(char *str,struct data_st* dt)
{
	int i=0;
	while(str[i]!=',')
	{
		dt->n1=(dt->n1*10) + (str[i]-48);
		i++;
	}
	i++;
	while(str[i]!=',')
	{
		dt->n2=(dt->n2*10) + (str[i]-48);
		i++;
	}
	i++;
	switch(str[i])
        {
                case '+': dt->res=dt->n1+dt->n2;
			  break;
                case '-': dt->res=dt->n1-dt->n2;
			  break;
                case '*': dt->res=dt->n1*dt->n2;
                           ...
                           loff_t *offset)
{
    ...
    //string operation
    //int res=find_result(kernel_buffer);	
    //printk(KERN_INFO " string_driver result : %d \n",res);
    
    //string operation using structure
    struct data_st data;
    cal_result(kernel_buffer,&data);
    printk(KERN_INFO "string_driver result : %d\n",data.res);
